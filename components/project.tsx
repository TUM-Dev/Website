...
export const Project: React.FC<ProjectProps> = ({ name, description, link, tech }) => (
        ...
        <Card className="border-0 shadow-md group-hover:shadow-lg transition-shadow dark:bg-slate-700 dark:border-slate-600 bg-white group-hover:bg-slate-50 group-hover:dark:bg-slate-800">
            <CardHeader>
                <CardTitle className="flex items-center justify-between dark:text-white text-gray-900">
                    {name}
                    {link && <ExternalLink className="w-4 h-4 dark:text-gray-400 text-gray-500" />}
                </CardTitle>
            ...
            <CardContent>
                <div className="flex flex-wrap gap-2">
                    {tech.map((tech) => (
                        <Badge className="text-xs dark:bg-slate-600 dark:text-gray-300" key={tech} variant="secondary">
                            {tech}
                        </Badge>
                    ))}
                </div>
